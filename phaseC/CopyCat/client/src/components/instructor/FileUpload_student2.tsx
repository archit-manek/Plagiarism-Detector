 ...
 */
export default class FileUpload_student2 extends React.Component<FileUploadProps, FileUploadState>{
   ...
   */
  componentDidUpdate(prevProps: FileUploadProps, prevState: FileUploadState): void {
  ...
  onSubmit = async (e: any) => {
    ...
    try {
      const res = await axios.post('http://localhost:3001/upload/student2', formData, {
        headers: {
  ...
  render() {
    return (
      <form onSubmit={this.onSubmit}>
        {this.state.message && this.state.alert && <div className="alert alert-info alert-dismissible fade show" role="alert">
          {this.state.message}
